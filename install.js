...
const setupDatabase = async function (db) {
  ...
  await db.exec('CREATE TABLE foo (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, nameID VARCHAR(36), type VARCHAR(64), FOREIGN KEY (nameID) REFERENCES test(nameID))');
};
